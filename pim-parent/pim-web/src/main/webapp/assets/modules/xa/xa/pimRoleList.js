$(function () {
	isPermission($(".js-add"), "xa:pimRole:add");
	isPermission($(".js-edit"), "xa:pimRole:edit");
	isPermission($(".js-delete"), "xa:pimRole:del");
	isPermission($(".js-search"), "xa:pimRole:view");
    ...
    function isPermission(el, permission) {
	 ...
	 //test
    var testListApiCtx = codeTempletCtx+"/xa/pimRole/list";  //test
    var testDeleteApiCtx = codeTempletCtx+"/xa/pimRole/delete";  //删除
    var testDeleteBatchApiCtx = codeTempletCtx+"/xa/pimRole/deleteBatch"; //删除多个
  ...
  **/
  function setImgRadio(data, flag, obj) {
    ...
    $(".js-add").on("click", function () {
        openDialog('添加数据', staticCtx + '/modules/xa/xa/pimRoleAddForm.html?type=add&id=null', dialWidth, 'auto', '', reloadPage);
    ...
    $(".js-edit").on("click", function () {
        ...
        if (editFlag) {  //有勾选
            ...
            if (count != 1) {  //多个
              ...
              //  openDialog('修改数据', staticCtx + '/modules/xa/xa/pimRoleEditForm.html?type=multiple&id=' + parameter["ids"], dialWidth, 'auto', '', reloadPage);
				selectTableRow();
            }else {  //单个
                //当前行的数据
                var data = $('#contentTable').DataTable().row($("#contentTable tbody input[type=checkbox]:checked").parents('tr')).data();
                openDialog('修改数据', staticCtx + '/modules/xa/xa/pimRoleEditForm.html?type=single&id=' + data.id, dialWidth, 'auto', '', reloadPage);
