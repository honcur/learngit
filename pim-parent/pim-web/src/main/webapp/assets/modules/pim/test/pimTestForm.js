app.controller('addFormCtrl', ['$scope', '$rootScope', '$http', '$templateCache', '$compile', '$document', function ($scope, $rootScope, $http, $templateCache, $compile, $document) {
	...
	var testGetSingleDataApiCtx = codeTempletCtx+"/pim.test/pimTest/get";  //查询单个数据
	var testGetMultipleDataApiCtx = codeTempletCtx+"/pim.test/pimTest/getByIds"; //查询多个数据
	var testGetInitDataApiCtx = codeTempletCtx+"/pim.test/pimTest/init"; //获取下拉，单选，复选初始数据
	var testSaveApiCtx = codeTempletCtx+"/pim.test/pimTest/save";  //保存1个或0个
    var testSaveBatchApiCtx = codeTempletCtx+"/pim.test/pimTest/saveBatch";  //保存多个
	...
	window.doSubmit = function(index, param) {
		...
		var body = $('body');
                    ...
                    var trs = body.find(".js-tableRichtext").find(".js-contentDiv");
                        ...
                        if (tableLen > 1) {  //保存多个
                            saveData["pimTestList"] = [];

                            trs.each(function (i, item) {
                            	var saveDataObj = {};
                            	saveDataObj = saveDataFun($(item));
                                saveData["pimTestList"].push(saveDataObj);
                            });
    ...
    **/
    function saveDataFun(e) {
        var obj = {},
        	...
        	if (e.find(".js-name").length != 0) {
            	name = e.find(".js-name").val();
        	}
        	if (e.find(".js-address").length != 0) {
            	address = e.find(".js-address").val();
        	}
