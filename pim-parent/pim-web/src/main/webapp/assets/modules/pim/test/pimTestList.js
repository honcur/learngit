$(function () {
	isPermission($(".js-add"), "pim.test:pimTest:add");
	isPermission($(".js-edit"), "pim.test:pimTest:edit");
	isPermission($(".js-delete"), "pim.test:pimTest:del");
	isPermission($(".js-search"), "pim.test:pimTest:view");
    ...
    function isPermission(el, permission) {
	 ...
	 //test
    var testListApiCtx = codeTempletCtx+"/pim.test/pimTest/list";  //test
    var testDeleteApiCtx = codeTempletCtx+"/pim.test/pimTest/delete";  //删除
    var testDeleteBatchApiCtx = codeTempletCtx+"/pim.test/pimTest/deleteBatch"; //删除多个
    var testGetInitDataApiCtx = codeTempletCtx+"/pim.test/pimTest/init"; //获取下拉，单选，复选初始数据
  ...
  **/
  function setImgRadio(data, flag, obj) {
    ...
    $(".js-add").on("click", function () {
        openDialog('添加数据', staticCtx + '/modules/pim.test/pimTestAddForm.html?type=add&id=null', dialWidth, 'auto', '', reloadPage);
    ...
    $(".js-edit").on("click", function () {
        ...
        if (editFlag) {  //有勾选
            ...
            if (count != 1) {  //多个
              ...
              //  openDialog('修改数据', staticCtx + '/modules/pim.test/pimTestEditForm.html?type=multiple&id=' + parameter["ids"], dialWidth, 'auto', '', reloadPage);
				selectTableRow();
            }else {  //单个
                //当前行的数据
                var data = $('#contentTable').DataTable().row($("#contentTable tbody input[type=checkbox]:checked").parents('tr')).data();
                openDialog('修改数据', staticCtx + '/modules/pim.test/pimTestEditForm.html?type=single&id=' + data.id, dialWidth, 'auto', '', reloadPage);
    ...
    $(".js-delete").on("click", function () {
        ...
        if (removeFlag) {  //有勾选            
            ...
            var deleteParam = {};
            var jsonParam = {};
            jsonParam["pimTestList"] = [];
            rowParam.each(function (i, item) {
                var data = $('#contentTable').DataTable().row(item).data();
                var obj = {
										"id" : data.id
                };
                jsonParam["pimTestList"].push(obj);
            });
