app.controller('addFormCtrl', ['$scope', '$rootScope', '$http', '$templateCache', '$compile', '$document', function ($scope, $rootScope, $http, $templateCache, $compile, $document) {
	...
	var testGetSingleDataApiCtx = codeTempletCtx+"/product/pimRole/get";  //查询单个数据
	var testGetMultipleDataApiCtx = codeTempletCtx+"/product/pimRole/getByIds"; //查询多个数据
	var testGetInitDataApiCtx = codeTempletCtx+"/product/pimRole/init"; //获取下拉，单选，复选初始数据
	var testSaveApiCtx = codeTempletCtx+"/product/pimRole/save";  //保存1个或0个
    var testSaveBatchApiCtx = codeTempletCtx+"/product/pimRole/saveBatch";  //保存多个
