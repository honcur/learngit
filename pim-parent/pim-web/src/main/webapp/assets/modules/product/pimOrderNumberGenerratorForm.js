app.controller('addFormCtrl', ['$scope', '$rootScope', '$http', '$templateCache', '$compile', '$document', function ($scope, $rootScope, $http, $templateCache, $compile, $document) {
	...
	var testGetSingleDataApiCtx = codeTempletCtx+"/product/pimOrderNumberGenerrator/get";  //查询单个数据
	var testGetMultipleDataApiCtx = codeTempletCtx+"/product/pimOrderNumberGenerrator/getByIds"; //查询多个数据
	var testGetInitDataApiCtx = codeTempletCtx+"/product/pimOrderNumberGenerrator/init"; //获取下拉，单选，复选初始数据
	var testSaveApiCtx = codeTempletCtx+"/product/pimOrderNumberGenerrator/save";  //保存1个或0个
    var testSaveBatchApiCtx = codeTempletCtx+"/product/pimOrderNumberGenerrator/saveBatch";  //保存多个
	...
	window.doSubmit = function(index, param) {
		...
		var body = $('body');
                    ...
                    var trs = body.find(".js-tableRichtext").find(".js-contentDiv");
                        ...
                        if (tableLen > 1) {  //保存多个
                            saveData["pimOrderNumberGenerratorList"] = [];

                            trs.each(function (i, item) {
                            	var saveDataObj = {};
                            	saveDataObj = saveDataFun($(item));
                                saveData["pimOrderNumberGenerratorList"].push(saveDataObj);
                            });
