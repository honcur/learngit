$(function () {
	isPermission($(".js-add"), "product:pimProductEntity:add");
	isPermission($(".js-edit"), "product:pimProductEntity:edit");
	isPermission($(".js-delete"), "product:pimProductEntity:del");
	isPermission($(".js-search"), "product:pimProductEntity:view");
    ...
    function isPermission(el, permission) {
	 ...
	 //test
    var testListApiCtx = codeTempletCtx+"/product/pimProductEntity/list";  //test
    var testDeleteApiCtx = codeTempletCtx+"/product/pimProductEntity/delete";  //删除
    var testDeleteBatchApiCtx = codeTempletCtx+"/product/pimProductEntity/deleteBatch"; //删除多个
  ...
  **/
  function setImgRadio(data, flag, obj) {
    ...
    $(".js-add").on("click", function () {
        openDialog('添加数据', staticCtx + '/modules/product/pimProductEntityAddForm.html?type=add&id=null', dialWidth, 'auto', '', reloadPage);
    ...
    $(".js-edit").on("click", function () {
        ...
        if (editFlag) {  //有勾选
            ...
            if (count != 1) {  //多个
              ...
              //  openDialog('修改数据', staticCtx + '/modules/product/pimProductEntityEditForm.html?type=multiple&id=' + parameter["ids"], dialWidth, 'auto', '', reloadPage);
				selectTableRow();
            }else {  //单个
                //当前行的数据
                var data = $('#contentTable').DataTable().row($("#contentTable tbody input[type=checkbox]:checked").parents('tr')).data();
                openDialog('修改数据', staticCtx + '/modules/product/pimProductEntityEditForm.html?type=single&id=' + data.id, dialWidth, 'auto', '', reloadPage);
    ...
    $(".js-delete").on("click", function () {
        ...
        if (removeFlag) {  //有勾选            
            ...
            var deleteParam = {};
            var jsonParam = {};
            jsonParam["pimProductEntityList"] = [];
            rowParam.each(function (i, item) {
                var data = $('#contentTable').DataTable().row(item).data();
                var obj = {
										"id" : data.id
                };
                jsonParam["pimProductEntityList"].push(obj);
            });
