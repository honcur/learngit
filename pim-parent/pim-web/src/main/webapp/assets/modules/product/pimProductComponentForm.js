app.controller('addFormCtrl', ['$scope', '$rootScope', '$http', '$templateCache', '$compile', '$document', function ($scope, $rootScope, $http, $templateCache, $compile, $document) {
	...
	var testGetSingleDataApiCtx = codeTempletCtx+"/product/pimProductComponent/get";  //查询单个数据
	var testGetMultipleDataApiCtx = codeTempletCtx+"/product/pimProductComponent/getByIds"; //查询多个数据
	var testGetInitDataApiCtx = codeTempletCtx+"/product/pimProductComponent/init"; //获取下拉，单选，复选初始数据
	var testSaveApiCtx = codeTempletCtx+"/product/pimProductComponent/save";  //保存1个或0个
    var testSaveBatchApiCtx = codeTempletCtx+"/product/pimProductComponent/saveBatch";  //保存多个
	...
	window.doSubmit = function(index, param) {
		...
		var body = $('body');
                    ...
                    var trs = body.find(".js-tableRichtext").find(".js-contentDiv");
                        ...
                        if (tableLen > 1) {  //保存多个
                            saveData["pimProductComponentList"] = [];

                            trs.each(function (i, item) {
                            	var saveDataObj = {};
                            	saveDataObj = saveDataFun($(item));
                                saveData["pimProductComponentList"].push(saveDataObj);
                            });
    ...
    **/
    function saveDataFun(e) {
        var obj = {},
        	...
        	if (e.find(".js-componentId").length != 0) {
            	componentId = e.find(".js-componentId").val();
        	}
        	if (e.find(".js-amount").length != 0) {
            	amount = e.find(".js-amount").val();
        	}
        	if (e.find(".js-description").length != 0) {
            	description = e.find(".js-description").val();
        	}
        	if (e.find(".js-purpose").length != 0) {
            	purpose = e.find(".js-purpose").val();
        	}
