$(function () {
	isPermission($(".js-add"), "product.category:pimProductCategory:add");
	isPermission($(".js-edit"), "product.category:pimProductCategory:edit");
	isPermission($(".js-delete"), "product.category:pimProductCategory:del");
	isPermission($(".js-search"), "product.category:pimProductCategory:view");
    ...
    function isPermission(el, permission) {
	 ...
	 //test
    var testListApiCtx = codeTempletCtx+"/product.category/pimProductCategory/list";  //test
    var testDeleteApiCtx = codeTempletCtx+"/product.category/pimProductCategory/delete";  //删除
    var testDeleteBatchApiCtx = codeTempletCtx+"/product.category/pimProductCategory/deleteBatch"; //删除多个
  ...
  **/
  function setImgRadio(data, flag, obj) {
    ...
    $(".js-add").on("click", function () {
        openDialog('添加数据', staticCtx + '/modules/product.category/pimProductCategoryAddForm.html?type=add&id=null', dialWidth, 'auto', '', reloadPage);
    ...
    $(".js-edit").on("click", function () {
        ...
        if (editFlag) {  //有勾选
            ...
            if (count != 1) {  //多个
              ...
              //  openDialog('修改数据', staticCtx + '/modules/product.category/pimProductCategoryEditForm.html?type=multiple&id=' + parameter["ids"], dialWidth, 'auto', '', reloadPage);
				selectTableRow();
            }else {  //单个
                //当前行的数据
                var data = $('#contentTable').DataTable().row($("#contentTable tbody input[type=checkbox]:checked").parents('tr')).data();
                openDialog('修改数据', staticCtx + '/modules/product.category/pimProductCategoryEditForm.html?type=single&id=' + data.id, dialWidth, 'auto', '', reloadPage);
    ...
    $(".js-delete").on("click", function () {
        ...
        if (removeFlag) {  //有勾选            
            ...
            var deleteParam = {};
            var jsonParam = {};
            jsonParam["pimProductCategoryList"] = [];
            rowParam.each(function (i, item) {
                var data = $('#contentTable').DataTable().row(item).data();
                var obj = {
										"id" : data.id
                };
                jsonParam["pimProductCategoryList"].push(obj);
            });
