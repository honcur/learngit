$(function () {
	isPermission($(".js-add"), "product:pimSoftwareProductEntity:add");
	isPermission($(".js-edit"), "product:pimSoftwareProductEntity:edit");
	isPermission($(".js-delete"), "product:pimSoftwareProductEntity:del");
	isPermission($(".js-search"), "product:pimSoftwareProductEntity:view");
    ...
    function isPermission(el, permission) {
	 ...
	 //test
    var testListApiCtx = codeTempletCtx+"/product/pimSoftwareProductEntity/list";  //test
    var testDeleteApiCtx = codeTempletCtx+"/product/pimSoftwareProductEntity/delete";  //删除
    var testDeleteBatchApiCtx = codeTempletCtx+"/product/pimSoftwareProductEntity/deleteBatch"; //删除多个
  ...
  **/
  function setImgRadio(data, flag, obj) {
    ...
    $(".js-add").on("click", function () {
        openDialog('添加数据', staticCtx + '/modules/product/pimSoftwareProductEntityAddForm.html?type=add&id=null', dialWidth, 'auto', '', reloadPage);
    ...
    $(".js-edit").on("click", function () {
        ...
        if (editFlag) {  //有勾选
            ...
            if (count != 1) {  //多个
              ...
              //  openDialog('修改数据', staticCtx + '/modules/product/pimSoftwareProductEntityEditForm.html?type=multiple&id=' + parameter["ids"], dialWidth, 'auto', '', reloadPage);
				selectTableRow();
            }else {  //单个
                //当前行的数据
                var data = $('#contentTable').DataTable().row($("#contentTable tbody input[type=checkbox]:checked").parents('tr')).data();
                openDialog('修改数据', staticCtx + '/modules/product/pimSoftwareProductEntityEditForm.html?type=single&id=' + data.id, dialWidth, 'auto', '', reloadPage);
    ...
    $(".js-delete").on("click", function () {
        ...
        if (removeFlag) {  //有勾选            
            ...
            var deleteParam = {};
            var jsonParam = {};
            jsonParam["pimSoftwareProductEntityList"] = [];
            rowParam.each(function (i, item) {
                var data = $('#contentTable').DataTable().row(item).data();
                var obj = {
										"id" : data.id
                };
                jsonParam["pimSoftwareProductEntityList"].push(obj);
            });
