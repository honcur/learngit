app.controller('addFormCtrl', ['$scope', '$rootScope', '$http', '$templateCache', '$compile', '$document', function ($scope, $rootScope, $http, $templateCache, $compile, $document) {
	...
	var testGetSingleDataApiCtx = codeTempletCtx+"/product/pimSoftware/get";  //查询单个数据
	var testGetMultipleDataApiCtx = codeTempletCtx+"/product/pimSoftware/getByIds"; //查询多个数据
	var testGetInitDataApiCtx = codeTempletCtx+"/product/pimSoftware/init"; //获取下拉，单选，复选初始数据
	var testSaveApiCtx = codeTempletCtx+"/product/pimSoftware/save";  //保存1个或0个
    var testSaveBatchApiCtx = codeTempletCtx+"/product/pimSoftware/saveBatch";  //保存多个
	...
	if (type == "single") {  //单个
		...
		$http.get(codeGenxhl.test.singleData + '/' + id)  
			.success(function(response, status, headers, config){  
			      ...
			      var obj = {};
				  					obj["id"] = data.id;
					obj["softwareName"] = data.softwareName;
	...
	window.doSubmit = function(index, param) {
		...
		var body = $('body');
                    ...
                    var trs = body.find(".js-tableRichtext").find(".js-contentDiv");
                        ...
                        if (tableLen > 1) {  //保存多个
                            saveData["pimSoftwareList"] = [];

                            trs.each(function (i, item) {
                            	var saveDataObj = {};
                            	saveDataObj = saveDataFun($(item));
                                saveData["pimSoftwareList"].push(saveDataObj);
                            });
    ...
    **/
    function saveDataFun(e) {
        var obj = {},
        	...
        	if (e.find(".js-softwareName").length != 0) {
            	softwareName = e.find(".js-softwareName").val();
        	}
			...
			if (softwareName !=undefined && softwareName != "") {
