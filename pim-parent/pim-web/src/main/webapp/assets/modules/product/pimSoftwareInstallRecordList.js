$(function () {
	isPermission($(".js-add"), "product:pimSoftwareInstallRecord:add");
	isPermission($(".js-edit"), "product:pimSoftwareInstallRecord:edit");
	isPermission($(".js-delete"), "product:pimSoftwareInstallRecord:del");
	isPermission($(".js-search"), "product:pimSoftwareInstallRecord:view");
    ...
    function isPermission(el, permission) {
	 ...
	 //test
    var testListApiCtx = codeTempletCtx+"/product/pimSoftwareInstallRecord/list";  //test
    var testDeleteApiCtx = codeTempletCtx+"/product/pimSoftwareInstallRecord/delete";  //删除
    var testDeleteBatchApiCtx = codeTempletCtx+"/product/pimSoftwareInstallRecord/deleteBatch"; //删除多个
    var testGetInitDataApiCtx = codeTempletCtx+"/product/pimSoftwareInstallRecord/init"; //获取下拉，单选，复选初始数据
  ...
  **/
  function setImgRadio(data, flag, obj) {
    ...
    $(".js-add").on("click", function () {
        openDialog('添加数据', staticCtx + '/modules/product/pimSoftwareInstallRecordAddForm.html?type=add&id=null', dialWidth, 'auto', '', reloadPage);
    ...
    $(".js-edit").on("click", function () {
        ...
        if (editFlag) {  //有勾选
            ...
            if (count != 1) {  //多个
              ...
              //  openDialog('修改数据', staticCtx + '/modules/product/pimSoftwareInstallRecordEditForm.html?type=multiple&id=' + parameter["ids"], dialWidth, 'auto', '', reloadPage);
				selectTableRow();
            }else {  //单个
                //当前行的数据
                var data = $('#contentTable').DataTable().row($("#contentTable tbody input[type=checkbox]:checked").parents('tr')).data();
                openDialog('修改数据', staticCtx + '/modules/product/pimSoftwareInstallRecordEditForm.html?type=single&id=' + data.id, dialWidth, 'auto', '', reloadPage);
    ...
    $(".js-delete").on("click", function () {
        ...
        if (removeFlag) {  //有勾选            
            ...
            var deleteParam = {};
            var jsonParam = {};
            jsonParam["pimSoftwareInstallRecordList"] = [];
            rowParam.each(function (i, item) {
                var data = $('#contentTable').DataTable().row(item).data();
                var obj = {
										"id" : data.id
                };
                jsonParam["pimSoftwareInstallRecordList"].push(obj);
            });
