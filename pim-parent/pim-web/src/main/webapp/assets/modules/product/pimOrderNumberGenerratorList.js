$(function () {
	isPermission($(".js-add"), "product:pimOrderNumberGenerrator:add");
	isPermission($(".js-edit"), "product:pimOrderNumberGenerrator:edit");
	isPermission($(".js-delete"), "product:pimOrderNumberGenerrator:del");
	isPermission($(".js-search"), "product:pimOrderNumberGenerrator:view");
    ...
    function isPermission(el, permission) {
	 ...
	 //test
    var testListApiCtx = codeTempletCtx+"/product/pimOrderNumberGenerrator/list";  //test
    var testDeleteApiCtx = codeTempletCtx+"/product/pimOrderNumberGenerrator/delete";  //删除
    var testDeleteBatchApiCtx = codeTempletCtx+"/product/pimOrderNumberGenerrator/deleteBatch"; //删除多个
    var testGetInitDataApiCtx = codeTempletCtx+"/product/pimOrderNumberGenerrator/init"; //获取下拉，单选，复选初始数据
  ...
  **/
  function setImgRadio(data, flag, obj) {
    ...
    $(".js-add").on("click", function () {
        openDialog('添加数据', staticCtx + '/modules/product/pimOrderNumberGenerratorAddForm.html?type=add&id=null', dialWidth, 'auto', '', reloadPage);
    ...
    $(".js-edit").on("click", function () {
        ...
        if (editFlag) {  //有勾选
            ...
            if (count != 1) {  //多个
              ...
              //  openDialog('修改数据', staticCtx + '/modules/product/pimOrderNumberGenerratorEditForm.html?type=multiple&id=' + parameter["ids"], dialWidth, 'auto', '', reloadPage);
				selectTableRow();
            }else {  //单个
                //当前行的数据
                var data = $('#contentTable').DataTable().row($("#contentTable tbody input[type=checkbox]:checked").parents('tr')).data();
                openDialog('修改数据', staticCtx + '/modules/product/pimOrderNumberGenerratorEditForm.html?type=single&id=' + data.id, dialWidth, 'auto', '', reloadPage);
    ...
    $(".js-delete").on("click", function () {
        ...
        if (removeFlag) {  //有勾选            
            ...
            var deleteParam = {};
            var jsonParam = {};
            jsonParam["pimOrderNumberGenerratorList"] = [];
            rowParam.each(function (i, item) {
                var data = $('#contentTable').DataTable().row(item).data();
                var obj = {
                };
                jsonParam["pimOrderNumberGenerratorList"].push(obj);
            });
