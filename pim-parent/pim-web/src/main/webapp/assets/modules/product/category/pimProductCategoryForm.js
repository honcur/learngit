app.controller('addFormCtrl', ['$scope', '$rootScope', '$http', '$templateCache', '$compile', '$document', function ($scope, $rootScope, $http, $templateCache, $compile, $document) {
	...
	var testGetSingleDataApiCtx = codeTempletCtx+"/product.category/pimProductCategory/get";  //查询单个数据
	var testGetMultipleDataApiCtx = codeTempletCtx+"/product.category/pimProductCategory/getByIds"; //查询多个数据
	var testGetInitDataApiCtx = codeTempletCtx+"/product.category/pimProductCategory/init"; //获取下拉，单选，复选初始数据
	var testSaveApiCtx = codeTempletCtx+"/product.category/pimProductCategory/save";  //保存1个或0个
    var testSaveBatchApiCtx = codeTempletCtx+"/product.category/pimProductCategory/saveBatch";  //保存多个
		...
		$http.get(codeGenxhl.test.singleData + '/' + id)  
			.success(function(response, status, headers, config){  
			      ...
			      var obj = {};
					...
					obj["categoryNumber"] = data.categoryNumber;
					obj["categorySource"] = data.categorySource;
	...
	window.doSubmit = function(index, param) {
		...
		var body = $('body');
                    ...
                    var trs = body.find(".js-tableRichtext").find(".js-contentDiv");
                        ...
                        if (tableLen > 1) {  //保存多个
                            saveData["pimProductCategoryList"] = [];

                            trs.each(function (i, item) {
                            	var saveDataObj = {};
                            	saveDataObj = saveDataFun($(item));
                                saveData["pimProductCategoryList"].push(saveDataObj);
                            });
    ...
    **/
    function saveDataFun(e) {
        var obj = {},
        	...
        	if (e.find(".js-categoryName").length != 0) {
            	categoryName = e.find(".js-categoryName").val();
        	}
        	if (e.find(".js-categoryNumber").length != 0) {
            	categoryNumber = e.find(".js-categoryNumber").val();
        	}
        	if (e.find(".js-categorySource").length != 0) {
            	categorySource = e.find(".js-categorySource").val();
        	}
        	if (e.find(".js-description").length != 0) {
            	description = e.find(".js-description").val();
        	}
        	if (e.find(".js-isShow").length != 0) {
            	isShow = e.find(".js-isShow").val();
        	}
        	if (e.find(".js-sort").length != 0) {
            	sort = e.find(".js-sort").val();
        	}
        	if (e.find(".js-parent").length != 0) {
            	parent = e.find(".js-parent").val();
        	}
			...
			if (categoryNumber !=undefined && categoryNumber != "") {
            	obj["categoryNumber"] = categoryNumber;
        	}
			if (categorySource !=undefined && categorySource != "") {
