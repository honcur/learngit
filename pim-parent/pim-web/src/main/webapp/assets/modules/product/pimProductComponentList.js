$(function () {
	isPermission($(".js-add"), "product:pimProductComponent:add");
	isPermission($(".js-edit"), "product:pimProductComponent:edit");
	isPermission($(".js-delete"), "product:pimProductComponent:del");
	isPermission($(".js-search"), "product:pimProductComponent:view");
    ...
    function isPermission(el, permission) {
	 ...
	 //test
    var testListApiCtx = codeTempletCtx+"/product/pimProductComponent/list";  //test
    var testDeleteApiCtx = codeTempletCtx+"/product/pimProductComponent/delete";  //删除
    var testDeleteBatchApiCtx = codeTempletCtx+"/product/pimProductComponent/deleteBatch"; //删除多个
    var testGetInitDataApiCtx = codeTempletCtx+"/product/pimProductComponent/init"; //获取下拉，单选，复选初始数据
  ...
  **/
  function setImgRadio(data, flag, obj) {
    ...
    $(".js-add").on("click", function () {
        openDialog('添加数据', staticCtx + '/modules/product/pimProductComponentAddForm.html?type=add&id=null', dialWidth, 'auto', '', reloadPage);
    ...
    $(".js-edit").on("click", function () {
        ...
        if (editFlag) {  //有勾选
            ...
            if (count != 1) {  //多个
              ...
              //  openDialog('修改数据', staticCtx + '/modules/product/pimProductComponentEditForm.html?type=multiple&id=' + parameter["ids"], dialWidth, 'auto', '', reloadPage);
				selectTableRow();
            }else {  //单个
                //当前行的数据
                var data = $('#contentTable').DataTable().row($("#contentTable tbody input[type=checkbox]:checked").parents('tr')).data();
                openDialog('修改数据', staticCtx + '/modules/product/pimProductComponentEditForm.html?type=single&id=' + data.id, dialWidth, 'auto', '', reloadPage);
    ...
    $(".js-delete").on("click", function () {
        ...
        if (removeFlag) {  //有勾选            
            ...
            var deleteParam = {};
            var jsonParam = {};
            jsonParam["pimProductComponentList"] = [];
            rowParam.each(function (i, item) {
                var data = $('#contentTable').DataTable().row(item).data();
                var obj = {
                };
                jsonParam["pimProductComponentList"].push(obj);
            });
