$(function () {
	isPermission($(".js-add"), "product:pimOrderNumberGenerator:add");
	isPermission($(".js-edit"), "product:pimOrderNumberGenerator:edit");
	isPermission($(".js-delete"), "product:pimOrderNumberGenerator:del");
	isPermission($(".js-search"), "product:pimOrderNumberGenerator:view");
    ...
    function isPermission(el, permission) {
	 ...
	 //test
    var testListApiCtx = codeTempletCtx+"/product/pimOrderNumberGenerator/list";  //test
    var testDeleteApiCtx = codeTempletCtx+"/product/pimOrderNumberGenerator/delete";  //删除
    var testDeleteBatchApiCtx = codeTempletCtx+"/product/pimOrderNumberGenerator/deleteBatch"; //删除多个
  ...
  **/
  function setImgRadio(data, flag, obj) {
    ...
    $(".js-add").on("click", function () {
        openDialog('添加数据', staticCtx + '/modules/product/pimOrderNumberGeneratorAddForm.html?type=add&id=null', dialWidth, 'auto', '', reloadPage);
    ...
    $(".js-edit").on("click", function () {
        ...
        if (editFlag) {  //有勾选
            ...
            if (count != 1) {  //多个
              ...
              //  openDialog('修改数据', staticCtx + '/modules/product/pimOrderNumberGeneratorEditForm.html?type=multiple&id=' + parameter["ids"], dialWidth, 'auto', '', reloadPage);
				selectTableRow();
            }else {  //单个
                //当前行的数据
                var data = $('#contentTable').DataTable().row($("#contentTable tbody input[type=checkbox]:checked").parents('tr')).data();
                openDialog('修改数据', staticCtx + '/modules/product/pimOrderNumberGeneratorEditForm.html?type=single&id=' + data.id, dialWidth, 'auto', '', reloadPage);
    ...
    $(".js-delete").on("click", function () {
        ...
        if (removeFlag) {  //有勾选            
            ...
            var deleteParam = {};
            var jsonParam = {};
            jsonParam["pimOrderNumberGeneratorList"] = [];
            rowParam.each(function (i, item) {
                var data = $('#contentTable').DataTable().row(item).data();
                var obj = {
                };
                jsonParam["pimOrderNumberGeneratorList"].push(obj);
            });
